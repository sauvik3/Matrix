#ifndef __MATRIX_ERROR_MESSAGES_H_INCLUDED__
#define __MATRIX_ERROR_MESSAGES_H_INCLUDED__

#define STR_MATRIX_NO_ERROR "No Error"
#define STR_MATRIX_INCOMPATIBLE_ORDER "Incompatible Matrix Orders for Operation"
#define STR_MATRIX_NOT_SQUARE "Operation is only applicable for Square Matrices"
#define STR_MATRIX_NOT_INITIALIZED "Attempted to use Uninitialized Matrix"
#define STR_MATRIX_INVALID_INDEX "Invalid Matrix Index"

#endif